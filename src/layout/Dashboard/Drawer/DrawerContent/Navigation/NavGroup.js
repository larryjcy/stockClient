...
const NavGroup = ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel, sessionUser, memberAcls}) => {
  const theme = useTheme()
  const {getDrawerOpen, updateActiveHorizontalMenuId, getActiveHorizontalMenuId} = useUserContext()
  ...
  const selectedID = getActiveHorizontalMenuId() //  menuMaster.openedHorizontalItem
  ...
  const checkOpenForParent = (child, id) => {
    child.forEach((ele) => {
      ...
      if (ele.url === pathname) {
        updateActiveHorizontalMenuId(id)
      }
    })
  }
  ...
  const checkSelectedOnload = (data) => {
    ...
    childrens.forEach((itemCheck) => {
      ...
      if (itemCheck?.url === pathname) {
        updateActiveHorizontalMenuId(currentItem.id)
      }
    })
  }
